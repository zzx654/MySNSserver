...
var db_config ={
    host: 'portfolio.cvponpcwczgo.us-east-1.rds.amazonaws.com',
    user: "root",
    database: "socialanony",
            ...
            connection.query(getmy,[platform,account],function(err,myresult){
                ...
                else
                {
                    if(latitude==undefined)
                    {
                        if(postnum==undefined)
                        {
                            param=[myresult[0].userid,myresult[0].userid,myresult[0].userid,""]
                            ...
                            " left outer join (select userid as id,nickname,profileimage from user) getuser on post.userid=getuser.id"+
                            " left outer join (select postid,count(*) as votecount from vote group by postid) vote on post.postid=vote.postid"+
                            " where userid not in (select blockeduserid from block where userid=?) and userid not in (select userid from block where blockeduserid=?) and userid in (select userid from follow where follower=?) and anonymous=? order by date desc,postnum desc limit 20"
                        }
                        else
                        {
                            param=[postdate,postdate,postnum,myresult[0].userid,myresult[0].userid,myresult[0].userid,""]
                            query="select post.postnum,post.postid,post.vote,post.userid,getuser.nickname,getuser.profileimage,post.anonymous,post.text,tag.tags,post.date,post.image,"
                            +"post.audio,ifnull(com.commentcount,0) as commentcount,ifnull(lik.likecount,0) as likecount,ifnull(vote.votecount,0) as votecount from post left outer"
                            +" join (select postid,count(*) as commentcount from comment group by postid) com on post.postid=com.postid left outer"+
                            " join (select postid,count(*) as likecount from likepost group by postid) lik on post.postid=lik.postid"+
                            ...
                            " left outer join (select userid as id,nickname,profileimage from user) getuser on post.userid=getuser.id"+
                            " left outer join (select postid,count(*) as votecount from vote group by postid) vote on post.postid=vote.postid"+
                            " where (date<? or (date=? and postnum<?)) and userid not in (select blockeduserid from block where userid=?) and userid not in (select userid from block where blockeduserid=?) and userid in (select userid from follow where follower=?)  and anonymous=?  order by date desc,postnum desc limit 20"
                    ...
                    else
                    {
                        if(postnum==undefined)
                        {
                            param=[latitude,longitude,latitude,myresult[0].userid,myresult[0].userid,myresult[0].userid,""]
                        ...
                        {
                            param=[latitude,longitude,latitude,postdate,postdate,postnum,myresult[0].userid,myresult[0].userid,myresult[0].userid,""]
                            query="select post.postnum,post.postid,post.vote,post.userid,getuser.nickname,getuser.profileimage,post.anonymous,post.text,tag.tags,post.date,post.image,"
                            +"post.audio,ifnull(com.commentcount,0) as commentcount,ifnull(lik.likecount,0) as likecount,if(isnull(post.latitude),-100.0,(6371*acos(cos(radians(?))*cos(radians(post.latitude))*cos"+
                            "(radians(post.longitude)-radians(?))+sin(radians(?))*sin(radians(post.latitude))))) as distance,ifnull(vote.votecount,0) as votecount from post left outer"
                            +" join (select postid,count(*) as commentcount from comment group by postid) com on post.postid=com.postid left outer"+
                            ...
                            " left outer join (select userid as id,nickname,profileimage from user) getuser on post.userid=getuser.id"+
                            " left outer join (select postid,count(*) as votecount from vote group by postid) vote on post.postid=vote.postid"+
                            " where (date<? or (date=? and postnum<?)) and userid not in (select userid from block where blockeduserid=?) and userid not in (select blockeduserid from block where userid=?) and userid in (select userid from follow where follower=?)  and anonymous=? order by date desc,postnum desc limit 20"
